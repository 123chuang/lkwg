...
- 鼠标曲线轨迹移动，可设置每次移动耗时、随机速度变化、点击前停顿、按压时间和启动前延迟
- 测试模式：只移动鼠标，不真正点击
- 全局热键：`F8` 启动/暂停，`F9` 停止
...
目标识别模式下，如果状态显示“未找到目标”，说明当前框选区域里没有匹配到模板图。可以重新框选模板、调低匹配阈值，或确认目标在点击区域内可见。

如果需要点“启动”后再切换到目标软件，保持“启动后最小化窗口”开启，并把“启动前延迟”设置为 2 到 5 秒。这个工具模拟的是系统鼠标，目标软件必须显示在屏幕上；已经最小化的窗口不能被这种方式后台点击。
...
- 如果目标程序以管理员身份运行，这个工具也需要以管理员身份运行。
